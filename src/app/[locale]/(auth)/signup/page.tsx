  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        // If email confirmation is enabled, we notify the user. If they are auto-logged in, redirect.
        if (data.session) {
          ...
          setIsSent(true)
          toast.success(t('auth.verification_sent') || 'Verification email sent!')
        }
      }
    } catch (err) {
    ...
    return (
      <div className="min-h-[calc(100vh-4rem)] bg-hero-gradient-subtle flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div className="sm:mx-auto sm:w-full sm:max-w-md">
          ...
          >
            <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-blue-100 text-imperial-blue mb-4">
            ...
            <p className="text-slate-500 font-medium mb-6">
              {t('auth.verification_sent') || `We have sent a verification link to ${email}. Please check your inbox to activate your account.`}
            </p>
        ...
        >
          <div className="mb-8 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500 font-medium">
              Join Ramanujonomics and start building your future.
            </p>
          </div>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <button
              type="submit"
              className="btn-primary w-full flex items-center justify-center gap-2 group cursor-pointer mt-6"
              disabled={isLoading}
            >
              {isLoading ? (
          ...
          >
            ...
            <span>{t('auth.google_signin') || 'Sign up with Google'}</span>
          </button>
